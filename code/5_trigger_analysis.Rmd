...
nbctroll <- read_csv(here::here("data/_PRIVATE_NBC_Coho_TrollHarvest2001-2019.csv")) %>%
  ...
  group_by(Year, StatWeek) %>%
  summarise(NBCtrollcoho = sum(cohototal))
...
Does US troll CPUE in early weeks predict the CPUE for the rest of the season?  

```{r UStrollbackground, message=FALSE, warning=FALSE, fig.height=3}
...
indices_2000 %>%
  dplyr::select(UStrollCPUE, Tyee_cpue_lead1, Nass_coho_lead1) %>%
  correlation() %>% 
...
indices_2000 %>%
  dplyr::select(UStrollCPUE, NBCtrollcoho, Tyee_cpue, Nass_coho, HS_count) %>%
  correlation() %>% 
  mutate_at(c("r", "CI_low",	"CI_high",	"t"), round, 2) %>%
  knitr::kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
...
mod_HS <- lm(UStrollCPUE ~ HS_count, data = indices_2000)
...
acf(indices_2000$UStrollCPUE, na.action = na.exclude, main = "Autocorr - Troll CPUE - 2000-2019")
...
pacf(indices_2000$UStrollCPUE, na.action = na.exclude, main = "Partial autocorr - Troll CPUE - 2000-2019")
```
...
acf((indices %>% filter(Year > 1990))$UStrollCPUE, na.action = na.exclude, main = "Autocorr - Troll CPUE - 1990-2019")

```
 
Summary: There is little autocorrelation, though we do see a lag of 3 for Tyee. It doesn't appear to be substantial and we will ignore it. (When we included through week 30, we see less autocorrelation).  
...
```{r troll2020, warning=FALSE, message=FALSE}
UStroll_cpue2020 <- read_csv(here::here("data/_PRIVATE_SEAK_Coho_TrollFPD_1981-2020_July20.csv"), 
                           guess_max = 84000) %>% #increased guess b/c of many blanks 
  rename("Gear" = `Gear Code`,
         "SellDate" = `Sell Date`, 
         "StatWeek" = `Stat Week`, 
...
allyr_summary <- UStroll_cpue2020 %>% 
  filter(StatWeek <= 29, # exclude week 30, based on treaty language
         StatArea != 101-85, StatArea != 101-90, StatArea != 101-95) %>%
  group_by(Year) %>%
...
```{r meanofamean, warning=FALSE, message=FALSE, fig.height=3}
meanofmean <- UStroll_cpue2020 %>% 
  filter(StatWeek <= 29, # exclude week 30, based on treaty language
         StatArea != 101-85, StatArea != 101-90, StatArea != 101-95) %>%
  group_by(Year, StatWeek) %>%
  summarise(meanCPUE = mean(CohoCPUE)) %>%
...
One reason to take the "mean of a mean" might be to weight harvest by stat week. If this is the objective, we recommend explicitly stating weights.  
...
Thus, if producing an annual estimate, basing it on the overall harvest will given a truer representation of the season's harvest. If using the mean of a mean, it should be noted how it affects the end result. The PSC Treaty language does not define how to take an average; we interpret this as meaning an overall average rather than means of weekly averages. Future establishment of changes in treaty triggers will need to account for different methods in averaging and require further assessment.  
