...
troll81to19 %>% 
  ...
  ggplot(aes(x=trollCPUE, y=trollCPUEallwk, fill=Year)) +
...
ggplot(trollcpue_wildabund, aes(x=trollCPUE, y = EstTotalWildAbund, fill = numtrips)) +
  #geom_text(aes(label = Year)) +
  geom_smooth(method = "lm") +
  geom_point(size = 3, pch = 21, color = "black") +
  geom_point(data = trollcpue_wildabund %>% filter(Year == 2019), 
             aes(x=trollCPUE, y = EstTotalWildAbund), pch = 21, size = 3, color = "black", fill = "red") +
  geom_point(data = trollcpue_wildabund %>% filter(Year == 2018), 
             aes(x=trollCPUE, y = EstTotalWildAbund), pch = 21, size = 3, color = "black", fill = "orange") +
...
indices_2000 %>%
  dplyr::select(trollCPUE, Tyee_cpue_lead1, Nass_coho_lead1) %>%
  correlation() %>% 
...
indices_2000 %>%
  dplyr::select(trollCPUE,Tyee_cpue, Nass_coho, HS_count) %>%
  correlation() %>% 
  mutate_at(c("r", "CI_low",	"CI_high",	"t"), round, 2) %>%
  knitr::kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
...
tyeetroll <- indices_2000 %>%
  ggplot(aes(x = trollCPUE, y = Tyee_cpue, fill = Year)) +
...
tyeelead <- indices_2000 %>%
  ggplot(aes(x = trollCPUE, y = Tyee_cpue_lead1, fill = Year)) +
  geom_smooth(method = "lm", color = "black") + 
  geom_point(pch = 21, color="black", size = 3) +
  scale_fill_adfg("glacier", discrete = FALSE) +
...
nasstroll <- indices_2000 %>%
  ggplot(aes(x = trollCPUE, y = Nass_coho, fill = Year)) +
  geom_smooth(method = "lm", color = "black") + 
  geom_point(pch = 21, color="black", size = 3) +
  scale_fill_adfg("glacier", discrete = FALSE) +
  scale_y_continuous(labels = scales::comma) +
...
nasslead <- indices_2000 %>%
  ggplot(aes(x = trollCPUE, y = Nass_coho_lead1, fill = Year)) +
  geom_smooth(method = "lm", color = "black") + 
  geom_point(pch = 21, color="black", size = 3) +
  scale_fill_adfg("glacier", discrete = FALSE) +
  scale_y_continuous(labels = scales::comma) +
...
mod_nasswk <- lm(trollCPUE ~ Nass_coho + week, data = indices_2000)
mod_tyeewk <- lm(trollCPUE ~ Tyee_cpue + week, data = indices_2000)
