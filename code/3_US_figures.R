...
US_Fig3 <- SEAK_sport %>%
   ...
   theme(legend.position=c(.2,.85), legend.title = element_blank(), legend.text = element_text(size = 10),
         legend.key.size = unit(1,"line"))
#ggsave(US_Fig3, filename = here::here("output/US_Fig3.png"), width = 6.5, height = 4, units = "in")


###### Figure 4 ######
# Inside rivers escapement
(Fig4a <- create_figure5("Auke Creek", setbreaks = c(0, 500, 1000, 1500)) + 
...
Fig5a <- create_figure5("Sitka Survey Index", setbreaks = seq(from=0, to=3000, by=500), minyear = 1982) +
   annotate("text", x = 2000, y = 3000, label = "Sitka Index (Survey)", size = 3.5) + 
   labs(y = "Spawners")

Fig5b <- create_figure5("Ford Arm Lake", setbreaks = seq(from=0, to=8000, by=1000), 
               minyear = 1982, blank_x = FALSE) +
   annotate("text", x = 2000, y = 8000, label = "Ford Arm Creek (Weir/Mark-recapture)", size = 3.5) + 
   labs(y = "Spawners")

US_Fig5 <- Fig5a / Fig5b
#ggsave(US_Fig5, filename = here::here("output/US_Fig5.png"), width = 6, height = 6, units = "in")
rm(Fig5a, Fig5b)


###### Figure 6 ######
# Ketchikan streams escapement
Fig6a <- create_figure5("Ketchikan Survey Index", setbreaks = seq(from=0, to=18000, by=2000), minyear = 1987) +
   annotate("text", x = 2000, y = 16000, label = "Ketchikan Index (Survey)", size = 3.5) + 
   labs(y = "Spawners")

Fig6b <- create_figure5("Hugh Smith Lake", setbreaks = seq(from=0, to=4000, by=500), 
                        minyear = 1982, blank_x = FALSE) +
...
Fig7a <- create_figure5("Tawah Creek", setbreaks = seq(from=0, to=10000, by=2500), minyear = 1972) +
   annotate("text", x = 1995, y = 10000, label = "Tawah Creek", size = 3.5) + 
   scale_x_continuous(breaks = seq(from = 1972, to = 2019, by = 2)) 

Fig7b <- create_figure5("Situk River", setbreaks = seq(from=0, to=40000, by=10000), minyear = 1972) +
...
Fig8c <- create_harvestfig(river = "Hugh Smith Lake", blank_x = FALSE, setbreaks = seq(from=0, to=9000, by=1000)) + 
   labs(y = "") +
   annotate("text", x = 2000, y = 10000, label = "Hugh Smith Lake", size = 3.5) 

US_Fig8 <- Fig8a / Fig8b / Fig8c 
#ggsave(US_Fig8, filename = here::here("output/US_Fig8.png"), width = 6.5, height = 8, units = "in")
...
Fig9a <- SEAK_escape %>% filter(River == "Chilkat River", Year >= 1987) %>% 
...
US_Fig10 <- taku_harvest_can %>%
   ...
   theme(legend.position=c(.85,.85), legend.title = element_blank(), legend.text = element_text(size = 10),
         legend.key.size = unit(1,"line")) 
#ggsave(US_Fig10, filename = here::here("output/US_Fig10.png"), width = 6.5, height = 4, units = "in")
...
US_Fig11 <- ggplot(totalchange, aes(x = Year, y = baselinediff, fill = posneg)) +
...
#ggsave(US_Fig11, filename = here::here("output/US_Fig11.png"), width = 6.5, height = 8, units = "in")
...
US_Fig19 <- SEAK_escape %>%
   ...
   theme(legend.position=c(.5,.06), legend.title = element_blank(), legend.text = element_text(size = 10),
         legend.key.size = unit(2.5,"line"), legend.direction="horizontal") 
#ggsave(US_Fig19, filename = here::here("output/US_Fig19.png"), width = 6.5, height = 4, units = "in")
...
###############################################
#### OTHER FIGURES ####
# These figures didn't make the cut but are still considered worthwhile. 
